...
\begin{todo}
\item compare program with game console: ``go there'' translates into presses of just a few buttons -- why?
\item compare program with drawing: ``draw a house'' translates into individual brush strokes -- why?
\item for the same reason, the program translates into fewer, more basic commands: machine code (or byte code)
\item what is relevant for running a program? -> memory for variables, line pointer, calculator for arithmetic, input and output, \dots
\item introduce von Neumann architecture (see figure \ref{fig_von_neumann})
\item what physical parts of a computer belong to which idealized ones?
\item VM is a simulation of such a machine (used for portable programs as additional abstraction layer)
\item look at simple Processing program \dots
\item (see figure \ref{fig_screenshot_vm_execution})
\item \dots
\item let students play ``Human Resource Machine'' \cite{Tom15}\footnote{The ``Hour of Code Edition'' is free to download for notebooks.} which introduces students to the limitations of machine language; suggestion: at least until level 6 for everybody, quicker students will find sufficient challenges later on (introduces jumps, memory access, arithmetic)
\item compare the command range with the one of Smalltalk bytecode
\item \dots
\end{todo}
...
\item Comparision with low level programming (e.\,g. \cite{Tom15})
\end{todo}
