...
The developers of \ac{GT} follow a trunk-only development style without release branches. This means that the release version changes almost daily, with new features being introduced gradually. This also means that subtle issues might be unexpectedly introduced in a release by or as a side-effect of some partially implemented feature. As a consequence, if \ac{GT} with an app is to be distributed, the best way to do this is by downloading the latest version, loading the app into it, testing it and then distributing \emph{this known good} image.

Additionally, \ac{GT} is mainly developed under macOS and makes some platform assumptions with relation to its host operating system. This isn't noteable when working purely within \ac{GT} but occasionally shows at its seams. As an example, at the start of this thesis, the code responsible for establishing a connection to the Python runtime failed to run under Windows. At the time of writing, this works better since this issue has been reported. Or the default database names include path separators which causes code finding a database by name to fail on systems using a different path separator.\footnote{Unix systems traditionally use a forward slash while Windows uses backslashes.}
