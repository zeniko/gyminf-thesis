...
GT is mainly developed under macOS and makes some platform assumptions with relation to its host operating system. This isn't noteable when working purely within GT but occasionally shows at its seams. Also, GT follows a trunk-only development style without release branches. This means that downloading GT on two different days might result in subtle differences. If GT with an app is to be distributed, the best way to do this is by downloading the latest version, loading the app into it, testing it and then distributing \emph{this known good} image (as is described in \ref{app_setup}).
