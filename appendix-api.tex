...
\item[\texttt{size(width, height)}] \hfill \\
	Prepares an output canvas of the given dimensions. This command must always be called first (for animations, it must be the first command in \ct{def setup}).
\end{description}
...
\item[\texttt{image(image, x, y)}, \texttt{image(image, x, y, width, height)}] \hfill \\
	Renders the image loaded with \ct{loadImage(...)} at the given coordinates (and scales it to fit the given size). All arguments except the first are identical to \ct{rect}'s. If \ct{width} and \ct{height} are not given, the image's native dimensions are used.
...
\item[\texttt{color(r, g, b)}, \texttt{color(gray)}] \hfill \\
	Generates a color value, which can be stored in a variable and can also be used as an argument for \ct{fill(...)}, \ct{stroke(...)}, and \ct{background(...)}.
...
\item[\texttt{def draw():}] \hfill \\
	Is called repeatedly (up to \ct{frameRate} times per second) to draw the output.
...
\item[\texttt{randomSeed(seed)}] \hfill \\
	Reinitializes the random generator with the given \ct{seed} number. Using the same number will result in the same exact sequence of pseudo-randomly generated numbers.
...
Note that for the following commands, \ct{list} is a variable referencing a list.
\begin{description}
...
\item[\texttt{print(value)}, \texttt{println(value)}] \hfill \\
	Prints the given value into an output console (mainly for debugging and for programs without graphical output).
\item[\texttt{str(value)}] \hfill \\
	Turns the value into a string, \eg to concatenate several values for use with \ct{text(...)}.
