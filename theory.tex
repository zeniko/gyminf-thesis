...
Starting from the observation that ``it was a fallacy to assume that students would be able to develop a working model of a computer [\dots] by designing small programs'' \cite[p.\,213]{Sch11},\footnote{German original: ``Es erwies sich als Irrtum, dass Schüler beim Entwerfen kleiner Programme ein tragfähiges kognitives Modell vom Rechner oder von Informatiksystemen im Allgemeinen entwickeln.''} Schubert and Schwill introduce the notion of \emph{Sichtenwechsel}, a change of perspective in relation to the current layer, which should help students better understand concepts of one layer by inspecting lower layers (p.\,173).\footnote{For lack of a concisely fitting English term, the German \emph{Sichtenwechsel} will be used throughout this thesis.}

Since multitier architectures and abstractions are foundational ideas of computer science, they propose to introduce and teach both gradually -- and that not only in the context of the networking stack and computer architecture (pp.\,113--116). As examples for doing so, they show a live model of a calculator app whose input is translated to both pseudocode and machine code (p.\,115); an environment for inspecting live Java objects (pp.\,208--209); or the Filius environment for inspecting a virtual computer network at its different layers (p.\,284, \cite{Gar15}).

Other authors, such as Jaokar \cite[p.\,51]{Jao12} or Zhang \cite[p.\,407]{Zha19}, have reached similar conclusions. This reinforces the need for combining programming \eg with computer architecture to allow students to perform a \emph{Sichtenwechsel}, which should help them to understand both better.
...
The didactic reasons for using both live and exploratory programming also apply outside of pure programming, such as when inspecting and modifying live systems, networks, simulations, \etc
...
In his article ``The Law of Leaky Abstractions'', Spolsky \cite{Spo02} introduces the concept of \emph{leaky abstractions}, claiming that for all non-trivial architectures, details of lower layers are to some degree bound to bleed through to upper layers. In other words, complex interfaces tend to be incomplete or ``leaky'' in practice.

In teaching computer science, such leaky abstractions occur repeatedly, \eg when an app doesn't run on a different device (with either the operating system or the processor architecture leaking), or when a document seemingly can't be saved (with either the file system or differences between apps leaking).
...
\item Source code consists of tokens, which are usually parsed into an \ac{AST}, which is either directly or via \ac{IR} translated into machine code, which runs on a virtual or actual machine.
...
\item If a program outputs emojis, they might look notably different in source code and output -- leaking font rendering.
...
\item If a program contains an endless loop, there might be neither an error message nor output, so that it might wrongly seem that the computer isn't doing anything. This is not an abstraction leak in the above sense but a related student misconception.
\end{itemize}
...
In cryptography, timing attacks have been successfully used for extracting passwords from insufficiently protected web servers \cite{Por18}. More recently, another class of timing attacks taking advantage of modern \ac{CPU}'s branch prediction optimizations has been demonstrated \cite{Koc19}. In the latter case, an implementation detail of the \ac{CPU} managed to leak. And in both cases, at least implementors of cryptographic programs must be aware of lower abstraction levels.
