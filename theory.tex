...
This environment for combining programming with a view on different abstraction layers should also adhere to current didactic best practices, among others manageability (\ref{ssc_manageability}), liveness and explorability (\ref{ssc_exploration}).
...
\item \cite{Lim24} Python on Arduino, but views missing
\item \cite{Oin25c}
\item \cite{Woe24} low-level block based programming
\item \cite{Kyf21} frame-based programming (shows AST interactively)
\item \cite{Mic25a} should but doesn't
\item Conclusion
\end{todo}
