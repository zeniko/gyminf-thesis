...
Before running it, create a new text file called \texttt{startup.st} in GT's top level folder (besides \texttt{GlamorousToolkit.image}) with the following content:
...
Finally run the \texttt{GlamorousToolkit} executable (under Windows and Linux it's located in the \texttt{bin} subfolder). The content of \texttt{Processing Abstractions} are now available behind the ``Unterrichtseinheiten'' home tile.


\chapter{GT Processing API} \label{app_api}

This appendix lists the available API calls implemented in Processing Abstraction's Processing. This has been auto-generated from GT page ``Processing API'' (\ct{db=5rgxfv9r684digmpw7ph7kbt4}):
