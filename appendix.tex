...
\item[\texttt{append(list, value)}] \hfill \\
	Appends the value to the end of the list and returns \emph{a new} list. (Note: \ct{list + [value]}is \emph{not yet} supported.)
\item[\texttt{concat(list, otherList)}] \hfill \\
	Combines the two lists to a single \emph{new} list. (Note: \ct{list + otherList} is \emph{not yet} supported.)
